{
  "translated": {
    "%s thinks Sentry can help monitor your mobile app": ["%s认为 Sentry 可以帮助监控你的移动端 App"],
    ...
    "Remove": ["移除"],
    "Recovery Codes": ["恢复代码"],
    "Recovery codes are the only way to access your account if you lose your device and cannot receive two factor authentication codes.": ["`恢复代码` 用于您丢失了设备并无法使用二步验证时进行访问"],
    "Activate": ["激活"],
    "View Codes": ["显示代码"],
    "Text Message": ["文本消息"],
    "This authenticator sends you text messages for verification.  It's useful as a backup method or when you do not have a phone that supports an authenticator application.": ["验证器已经给你发送了文本验证信息。当你没有支持验证器的手机时，这将是一个有用的后备方式。"],
    "A confirmation code was sent to %(phone_mask)s. It is valid for %(ttl)d seconds.": ["确认码已发送至%(phone_mask)s。%(ttl)d秒内有效。"],
    "Error: we failed to send a text message to you. You can try again later or sign in with a different method.": ["错误：给你发送文本信息时失败了。你可以再次尝试或使用其它方法登录。"],
    "%(code)s is your Sentry two-factor enrollment code. You are about to set up text message based two-factor authentication.": ["%(code)s 是您的 Sentry 双因子认证注册码。您即将建立基于文本消息的双因素身份验证。"],
    "%(code)s is your Sentry authentication code.": ["%(code)s 是你的 Sentry 授权码。"],
    "Authenticator App": ["验证器应用"],
    "An authenticator application that supports TOTP (like Google Authenticator or 1Password) can be used to access your account securely using a token and secret key. A new token is generated every 30 seconds.": ["支持TOTP的认证器应用程序（如谷歌认证器或1Password）可用于使用令牌和 键 安全地访问您的账户。每30秒就会生成一个新的令牌。"],
    "Your account is currently linked to an authenticator application. To link to a new device or application, or to update your secret key, click \"Confirm\" below. By clicking \"Confirm\", your existing secret key will be replaced and will no longer work to access your account.": ["你的帐号已经设置了多步认证。如需链接到新设备，或需更新密钥，请点击 \"确认\" 按钮。如果点击确认，您现有的密钥将被重置，且不能访问当前帐号。"],
    "Configure": ["配置"],
    "U2F (Universal 2nd Factor)": ["U2F (Universal 2nd Factor)"],
    "Authenticate with a U2F hardware device. This is a device like a Yubikey or something similar which supports FIDO's U2F specification. This also requires a browser which supports this system (like Google Chrome).": ["使用 U2F 硬件设备授权。这是一个类似 Yubikey 或其它支持 FIDO 的 U2F 本协议的设备。这还需要一个支持本系统的浏览器 (比如 Google Chrome)。"],
    "You have successfully linked your account to your SSO provider.": ["您已成功将您的账户链接至 SSO 提供程序。"],
    "SSO has been configured for your organization and any existing members have been sent an email to link their accounts.": ["您的组织已配置好SSO，我们已向所有成员发送邮件来链接他们的账户。"],
    "There was an error encountered during authentication.": ["身份验证出错。"],
    "You must be authenticated to link accounts.": ["如要链接账户，您必须先通过身份验证。"],
    "The provider did not return a valid user identity.": ["身份验证提供程序未返回合法的用户信息。"],
    ...
    "Continue": ["继续"],
    "The IdPs unique ID attribute key for the user. This is what Sentry will used to identify the users identity from the identity provider.": ["用户的 IdP 唯一 ID 属性键。 Sentry 将用它来从身份提供者识别用户身份。"],
    "The IdPs email address attribute key for the user. Upon initial linking this will be used to identify the user in Sentry.": ["用户的 IdP 电子邮件地址属性键。 在初始链接时，这将用于识别 Sentry 中的用户。"],
    ...
    "The organization does not exist or does not have SAML SSO enabled.": ["该组织不存在或没有启用单点登录\"SAML SSO\""],
    "SAML SSO failed, {reason}": ["单点登录SAML SSO失败，{reason}"],
    "Failed to map SAML attributes. Assertion returned the following attribute keys: {error_msg_keys}": ["映射 SAML 属性失败。请确保 SAML 服务返回了以下属性键：{error_msg_keys}"],
    ...
    "Configuration for the {name} integration has been saved.": ["对{name}集成的配置已保存。"],
    "Big Integer": ["大整数"],
    "'%s' is not a valid JSON string.": ["'%s' 不是有效JSON字符串"],
    "Your {identity_provider} account has been associated with your Sentry account": ["你的 {identity_provider} 账户已关联至 Sentry 账户"],
    "Path may not contain spaces or quotations": ["路径不能包含空格或引号"],
    "Installation": ["安装"],
    "Please validate the Cloudformation stack was created successfully": ["请确认 Cloudformation 中 stack 已成功创建"],
    "Unknown error": ["未知错误"],
    "Unknown Error": ["未知错误"],
    "Bitbucket URL": ["Bitbucket 地址"],
    "The base URL for your Bitbucket Server instance, including the host and protocol.": ["你的Bitbucket服务器的实例地址, 包括域名和协议."],
    "Verify SSL": ["验证SSL"],
    "By default, we verify SSL certificates when making requests to your Bitbucket instance.": ["默认情况下，向 Bitbucket 实例发起请求时我们会验证 SSL 证书。"],
    "Bitbucket Consumer Key": ["Bitbucket用户密钥"],
    "Bitbucket Consumer Private Key": ["Bitbucket用户私钥"],
    "It seems that your GitHub account has been installed on another Sentry organization. Please uninstall and try again.": ["您的 GitHub 帐户似乎已安装在另一个Sentry组织上。请卸载并重试。"],
    "It seems that your Sentry organization has an installation pending deletion. Please wait ~15min for the uninstall to complete and try again.": ["看来您的 Sentry 组织有一个待删除的安装。请等待 ~15分钟，待卸载完成后再试。"],
    "The \"base URL\" for your GitHub enterprise instance, includes the host and protocol.": ["你的GitLab Enterprise实例地址，包括域名和协议。"],
    "The App ID of your Sentry app. This can be found on your apps configuration page.": ["您Sentry应用的ID，可以在您项目配置也找到。"],
    "The GitHub App name of your Sentry app. This can be found on the apps configuration page.": ["您Sentry应用的GitHub应用名称，可以在应用配置页面找到。"],
    "By default, we verify SSL certificates when delivering payloads to your GitHub Enterprise instance": ["默认情况下，向 Github Enterprise 实例发起请求时我们会验证 SSL 证书。"],
    "We require a webhook secret to be configured. This can be generated as any random string value of your choice and should match your GitHub app configuration.": ["我们需要配置一个 webhook 密码。 可以是您选择生成的任何随机字符串值，并且应该与您的 GitHub 应用程序配置相匹配。"],
    "The Private Key generated for your Sentry GitHub App.": ["为您Sentry Github应该生成的私钥。"],
    "GitLab URL": ["GitLab地址"],
    "GitLab Group Path": ["GitLab组路径"],
    "my-group/my-subgroup": ["我的组/我的子组"],
    "Include Subgroups": ["包含子组"],
    "By default, we verify SSL certificates when delivering payloads to your GitLab instance, and request GitLab to verify SSL when it delivers webhooks to Sentry.": ["默认情况下，我们在向您的 GitLab 实例发送数据时会验证其 SSL 证书，并要求GitLab在通过Webhook 请求Sentry时验证 SSL证书。"],
    "GitLab Application ID": ["GitLab应用ID"],
    "5832fc6e14300a0d962240a8144466eef4ee93ef0d218477e55f11cf12fc3737": ["5832fc6e14300a0d962240a8144466eef4ee93ef0d218477e55f11cf12fc3737"],
    ...
    "When a Sentry issue changes status, change the status of the linked ticket in Jira.": ["当一个 Sentry 事件 改变了状态，同时 Jira 相应票据 链接 中的状态"],
    ...
    "When an issue is assigned in Sentry, assign its linked Jira ticket to the same user.": ["当一个 事件 在Sentry中被分派。在 Jira相应票据 链接 中分派给同一个用户。"],
    "Sync Sentry Comments to Jira": ["同步Sentry评论到Jira"],
    "Post comments from Sentry issues to linked Jira tickets": ["从Sentry发布评论到关联的Jira条目"],
    "Sync Jira Status to Sentry": ["同步Jira状态到Sentry"],
    "When a Jira ticket is marked done, resolve its linked issue in Sentry. When a Jira ticket is removed from being done, unresolve its linked Sentry issue.": ["当Jira中票据被标记完成，标记对应Sentry事件为解决。当Jira中票据移除已完成状态，标记对应Sentry事件为未解决。"],
    "Sync Jira Assignment to Sentry": ["同步 Jira 任务至 Sentry"],
    "When a ticket is assigned in Jira, assign its linked Sentry issue to the same user.": ["当一个工单在 Jira 中被指派时，指派其在 Sentry 中关联的问题给同一用户。"],
    "Unable to communicate with the Jira instance. You may need to reinstall the addon.": ["无法与现有 Jira 实例通讯，你可能需要重装 Jira 插件。"],
    "Your organization does not have access to this feature": ["您的组织没有权限访问这个特性"],
    "Jira URL": ["Jira地址"],
    "The base URL for your Jira Server instance, including the host and protocol.": ["您的Jira 服务实例地址，需要包含域名和协议。"],
    "By default, we verify SSL certificates when making requests to your Jira instance.": ["默认情况下，在向Jira实例发起请求的时候我们会验证 SSL证书。"],
    "Jira Consumer Key": ["Jira Consumer Key"],
    "Jira Consumer Private Key": ["Jira用户私钥（Jira Consumer Private Key）"],
    "The channel or user \"%(channel)s\" could not be found in the %(team)s Team.": ["在 %(team)s 团队中找不到频道或用户“%(channel)s”。"],
    "Teams Marketplace": ["团队市场"],
    "Invalid {field}": ["无效的 {field}"],
    "The service \"%(service)s\" has not been granted access in the %(account)s Pagerduty account.": ["服务%(service)s不存在，或在%(account)s Pageduty 账号中未被允许访问"],
    "The provided %(proper_name)s account is linked to a different Sentry user. To continue linking the current Sentry user, please use a different %(proper_name)s account.": ["提供的%(proper_name)s账户已关联了另一个 Sentry 用户。 要继续关联当前 Sentry 用户，请使用其他%(proper_name)s帐户。"],
    "Workspace": ["工作区"],
    ...
    "Visit the Vercel Marketplace to install this integration. After installing the Sentry integration, you'll be redirected back to Sentry to finish syncing Vercel and Sentry projects.": ["访问 Vercel 市场以安装此集成。安装 Sentry 集成后，您将被重定向回 Sentry 以完成 Vercel 和 Sentry 项目的同步。"],
    "Vercel Marketplace": ["Vercel 市场"],
    "Connect Your Projects": ["连接你的项目"],
    "Install a [source code integration]({}) and configure your repositories.": ["安装 [代码集成]({}) 并配置您的存储库。"],
    "Vercel project...": ["Vercel 项目..."],
    "Link your Sentry projects to complete your installation on Vercel": ["连接你的Sentry项目以完成Vercel的安装"],
    "Complete on Vercel": ["Vercel完成"],
    "Sync Sentry Status to Azure DevOps": ["同步Sentry状态到Azure DevOps"],
    "When a Sentry issue changes status, change the status of the linked work item in Azure DevOps.": ["当Sentry的问题状态变化时同步到Azure Devops的工作项中。"],
    ...
    "When an issue is assigned in Sentry, assign its linked Azure DevOps work item to the same user.": ["在 Sentry中分配问题时，将其链接的 Azure DevOps 工作项分配给同一用户。"],
    "Sync Sentry Comments to Azure DevOps": ["同步Sentry评论到Azure DevOps"],
    "Post comments from Sentry issues to linked Azure DevOps work items": ["将 Sentry 问题的评论发布到关联的 Azure DevOps 工作项中"],
    "Sync Azure DevOps Status to Sentry": ["同步 Azure DevOps 状态至 Sentry"],
    "When a Azure DevOps work item is marked done, resolve its linked issue in Sentry. When a Azure DevOps work item is removed from being done, unresolve its linked Sentry issue.": ["当 Azure DevOps 工作项被标记为完成时，将 Sentry 中关联的问题标记为已解决； 当 Azure DevOps 工作项被从已完成中删除时，将 Sentry 中关联的问题标记为未解决。"],
    "Sync Azure DevOps Assignment to Sentry": ["同步 Azure DevOps 任务至 Sentry"],
    "When a work item is assigned in Azure DevOps, assign its linked Sentry issue to the same user.": ["在 Azure DevOps 中分配工作项时，将其关联的 Sentry 问题分配给同一用户。"],
    "Project": ["项目"],
    "MyProject": ["我的项目"],
    ...
    "Enter callback URLs to POST new events to (one per line).": ["输入新事件将要 POST 的回调 URL（每行一个）。"],
    "CSRF Verification Failed": ["CSRF 验证失败"],
    "A required security token was not found or was invalid.": ["安全令牌不存在或无效"],
    "If you're continually seeing this issue, try the following:": ["如果您不断看到此问题，可尝试以下步骤："],
    "Clear cookies (at least for Sentry's domain).": ["清除 cookies（至少是 Sentry 域名下的 cookies）。"],
    "Reload the page you're trying to submit (don't re-submit data).": ["重新载入您试图提交的页面（不要重新提交数据）。"],
    "Re-enter the information, and submit the form again.": ["重新输入信息，然后再次提交表格。"],
    ...
    "You have confirmed your email, and may now update your password below.": ["您的邮件地址已经确认，现在可以修改您的密码了。"],
    "Change Password": ["修改密码"],
    "Password Expired": ["密码已过期"],
    "The password on your account expired.": ["您的帐户的密码已过期"],
    "We have sent an email to the address registered with this account containing further instructions to reset your password.": ["已经向该账户的邮箱发送一封带有指引的密码重置邮件。"],
    "We were unable to confirm your identity. Either the link you followed is invalid, or it has expired. You can always <a href=\"%(link)s\">try again</a>.": ["我们无法确认你的身份。你使用的邀请链接无效，或已过期。你可以<a href=\"%(link)s\">再次尝试</a>。"],
    "We will send a confirmation email to this address:": ["我们将向该地址发送确认电子邮件："],
    "Send Email": ["发送Email"],
    "Set Password": ["设置密码"],
    "Set your account password below.": ["在下方设置你的账户密码"],
    "This password link has expired. Request a new password recovery code to set\n\tyour account password": ["这个密码重置链接已经失效，请申请一个新的密码恢复码来重置你的账户密码"],
    "Confirm Password": ["确认密码"],
    "Confirm your Identity": ["确认你的身份"],
    "Help us keep your account safe by confirming your identity.": ["验证您的身份以协助我们确保您的账户安全"],
    ...
    "Sign out": ["退出"],
    "Fancy seeing you here. In your own inbox. In this email, we’ve addressed specifically to you.": ["很高兴认识你"],
    ...
    "These channels were found to be missing or unauthorized in your legacy Slack integration. If they were private channels, you'll have make sure to add the Sentry bot to these channels. Otherwise you may want update your alert rules accordingly.": ["在您的旧版 Slack 集成中发现这些渠道丢失或未经授权。 如果它们是您的私人频道，您必须确保将 Sentry 机器人添加到这些频道。 否则，您可能需要更新相应的报警规则。"],
    ...
    "Add your Bitbucket Server App credentials to Sentry.": ["添加你的 Bitbucket 服务应用证书到 Sentry。"],
    "Submit": ["提交"],
    ...
    "To configure GitLab with Sentry, you will need to create a Sentry app in your GitLab instance.": ["为了能够将Sentry配置到Gitlab , 您需要创建一个Sentry 应用到您的Gitlab 实例中."],
    "You'll also need to be a maintainer or owner in GitLab. Projects owned by users are not supported.": ["您还需要是 GitLab 的维护者或所有者。 不支持普通用户拥有的项目。"],
    "Navigate to the User Settings section of your GitLab instance.": ["导航至你的GitLab实例的用户设置区域。"],
    "In the resulting form, enter the following information:": ["在结果表单中，输入以下信息："],
    "Click Save Application.": ["点击保存应用"],
    "In the resulting page, you'll see the Application ID and Secret. You'll need those for the next phase of setup.": ["在结果页面中，您将看到应用程序 ID 和密钥（Application ID & Secret）。 您将需要这些用于下一阶段的设置。"],
    "You must complete the above steps in GitLab to connect GitLab with Sentry.": ["你必须在GitLab完成上述步骤，才能完成Sentry与GitLab的连接。"],
    "Next": ["下一个"],
    "This page has expired, please refresh to view the Sentry issue": ["此页面已过期，请刷新以查看Sentry问题"],
    "This Sentry issue is not linked to a Jira issue": ["这个 Sentry 问题没有链接到一个 Jira 问题"],
    "Date:": ["日期："],
    "Release:": ["释放："],
    "Jira-Server Setup": ["Jira-服务安装"],
    "Add your Jira Server App credentials to Sentry.": ["添加你的Jira Server应用证书到Sentry。"],
    "Microsoft Teams Expired Link": ["微软团队过期链接"],
    "Microsoft Teams Linked": ["微软团队链接"],
    "Your Microsoft Teams identity has been associated with your Sentry account. You may now take actions through Microsoft Teams.": ["您的 Microsoft Teams 身份已经关联你的 Sentry 帐户。现在你可以通过 Microsoft Teams 进行操作。"],
    "Microsoft Teams No Identities": ["微软团队 无身份"],
    "Your Microsoft Teams identity is not associated with this Sentry account.": ["您的 Microsoft 团队身份不与此Sentry帐户关联。"],
    "Confirm that you'd like to unlink your Microsoft Teams identity from your Sentry account.": ["确认你想要取消你的 Microsoft 团队身份与你的Sentry帐户的链接。"],
    "Unlink with Microsoft Teams": ["取消与微软团队的链接"],
    "Microsoft Teams Unlinked": ["微软团队未链接"],
    "Your Microsoft Teams identity has been unassociated with your Sentry account.": ["您的 Microsoft 团队身份已与您的Sentry帐户取消关联。"],
    "Slack Expired Link": ["Slack失效链接"],
    "Error": ["错误"],
    "Link Team": ["关联团队"],
    "Slack Linked": ["Slack已关联"],
    "Your Slack identity has been associated with your Sentry account. You may now take actions through Slack!": ["你的Slack身份已经关联至你的Sentry账户，现在你可以通过Slack进行操作"],
    "Slack Migration": ["Slack集成"],
    "Time For An Upgrade": ["是时候升级了"],
    "Well, look who has private channels with alert rules.": ["好吧，看看谁拥有带有警报规则的私有频道。"],
    "Next Steps": ["下一步"],
    ...
    "The time has come, Slack's deprecated its workspace apps. And with it, goes our legacy Slack integration.": ["时机已到，Slack 已弃用其工作区应用程序。 随之而来的是我们传统的 Slack 集成。"],
    "But don't worry, we built you a whole new Sentry-Slack integration. Just authorize the app, and then you can get back to doing whatever it is you came here to do.": ["但别担心，我们为您打造了一个全新的 Sentry-Slack 集成。 只需授权该应用程序，然后您就可以重新开始做您来到这里要做的任何事情。"],
    "Your existing alert rules may need some love.": ["您可能需要关注一下现有的警报规则。"],
    "If you don't have private channels in your alert rules, just authorize our new Slack app and then you can get on with your life.": ["如果你的警报规则中没有私人频道，只需授权我们新的 Slack 应用，然后就可以继续你的生活。"],
    "Once you've upgraded your integration, all other organizations with the same workspace will be upgraded as well.": ["升级集成后，具有相同工作区的所有其他组织也将升级。"],
    "Unlink Team": ["取消关联团队"],
    "Team Unlinked": ["团队 未关联"],
    "Slack Unlinked": ["Slack 未关联"],
    "Your Slack identity has been unassociated with your Sentry account.": ["您的Slack身份已与您的Sentry账户取消关联."],
    "Azure DevOps Setup": ["Azure DevOps安装"],
    "No accounts found. Please check that you are an Azure DevOps account owner.": ["没有找到帐户。 请检查您是否是 Azure DevOps 帐户的所有者。"],
    ...
    "You do not have access to any organizations within Sentry.": ["您无权访问Sentry内的任何组织"],
    "If you were expecting otherwise, this may be due to a configuration change in the organization. Please check your email inbox for an invite link to rejoin the organization.": ["如果您另有预期，这可能是由于组织中的配置更改。请检查您的 电子邮箱 收件箱以获取重新加入组织的邀请链接。"],
    "Alternatively, reach out to an admin in your organization to re-invite you using ": ["或者，联系您单位中的管理员，使用"],
    "this guide": ["这份指引"],
    "If you need to make changes to your user account, ": ["如果您需要对您的用户账户作出变更，"],
    ...
    "Create Issue": ["创建问题"],
    "Link Issue": ["连接问题"],
    "You still need to <a href=\"%(link)s\">associate an identity</a> with %(title)s before you can\n        create issues with this service.": ["你需要把<a href=\"%(link)s”>身份</a>与%(title)s绑定起来，才能使用这项服务创建事件。"],
    "Your server administrator will need to configure authentication with\n            <strong>%(auth_provider)s</strong> before you can use this plugin.": ["如需使用此插件，你的服务器管理员需要用<strong>%(auth_provider)s</strong>来配置验证服务。"],
    "You still need to <a href=\"%(plugin_link)s\">configure this plugin</a>\n                before you can use it.": ["您需要<a href=\"%(plugin_link)s\">配置插件</a>来使用它"],
    "Changes to your configuration were saved successfully.": ["对配置的修改已保存成功。"],
    ...
    "name": ["姓名"],
    "staff status": ["员工状态"],
    "Designates whether the user can log into this admin site.": ["是否允许该用户登陆该管理界面。"],
    "active": ["激活"],
    "Designates whether this user should be treated as active. Unselect this instead of deleting accounts.": ["是否使该用户处于激活状态。您可以取消选择而不是删除帐户。"],
    "superuser status": ["超级用户状态"],
    "Designates that this user has all permissions without explicitly assigning them.": ["是否允许该用户拥有所有未特别分配给他的权限。"],
    "managed": ["管理"],
    "Designates whether this user should be treated as managed. Select this to disallow the user from modifying their account (username, password, etc).": ["是否将这个用户指定为受管理账户。选择该项将防止用户修改其账户（用户名，密码等）。"],
    "is sentry app": ["是Sentry应用"],
    "Designates whether this user is the entity used for Permissionson behalf of a Sentry App. Cannot login or use Sentry like anormal User would.": ["指定此用户是否为代表 Sentry 应用程序用于 权限 的实体。 无法像普通用户那样登录或使用 Sentry。"],
    "password expired": ["密码已过期"],
    "If set to true then the user needs to change the password on next sign in.": ["如果设置为真，用户下次登录时必须修改密码。"],
    ...
    "users": ["用户"],
    "verified": ["已认证"],
    "Designates whether this user has confirmed their email.": ["指定该用户是否已确认 email。"],
    "Browser": ["浏览器"],
    "Server": ["服务器"],
    "Mobile": ["手机"],
    "Desktop": ["桌面"],
    ...
    "Password": ["密码"],
    "password": ["密码"],
    "Please enter a correct %(username)s and password. Note that both fields may be case-sensitive.": ["请输入正确的%(username)s和密码。请注意，用户名和密码是大小写敏感的。"],
    "You have made too many failed authentication attempts. Please try again later.": ["验证失败太多次，请稍后再试。"],
    "Your Web browser doesn't appear to have cookies enabled. Cookies are required for logging in.": ["您的浏览器似乎没用启用cookies。登录需要Cookies。"],
    "This account is inactive.": ["此账号已经失效。"],
    "Name": ["姓名"],
    "Email": ["电子邮件"],
    "Email updates": ["邮件更新"],
    "An account is already registered with that email address.": ["该电子邮件地址已经注册过账户。"],
    "The account you are trying to recover is managed and does not support password recovery.": ["你尝试找回的这个账户不支持密码找回功能。"],
    "New Email": ["新 Email"],
    "Current password": ["当前密码"],
    "You will need to enter your current account password to make changes.": ["需要输入当前密码来进行变更。"],
    "The password you entered is not correct.": ["输入的密码不正确。"],
    "You must confirm your current password to make changes.": ["需要确认当前密码来进行变更。"],
    "Authenticator code": ["认证码"],
    "Authenticator or recovery code": ["认证或恢复码"],
    "Invalid username": ["无效的用户名"],
    "Not set": ["未设置"],
    "There was an error confirming your email. Please try again or visit your Account Settings to resend the verification email.": ["确认电子邮件时发生错误。请进入你的账户设置重新发送验证邮件。"],
    ...
    "The organization does not exist or does not have Single Sign-On enabled.": ["该组织不存在或没有启用单点登录。"],
    "An unknown error occurred while submitting your report. Please try again.": ["提交报告时发生错误，请重试。"],
    "Some fields were invalid. Please correct the errors and try again.": ["某些字段填写无效。请更正错误并重试。"],
    ...
    "If you'd like to help, tell us what happened below.": ["如果您愿意帮忙，您可以在下方告诉我们发生了什么。"],
    "What happened?": ["发生了什么？"],
    "Close": ["关闭"],
    ...
    "The SSO feature is not enabled for this organization.": ["该组织未启用 SSO 登录功能。"],
    "SSO authentication has been disabled.": ["SSO 登录已禁用。"],
    "A reminder email has been sent to members who have not yet linked their accounts.": ["已向所有尚未链接账户的成员发送提醒邮件。"],
    "Require SSO": ["需要 SSO"],
    "Require members use a valid linked SSO account to access this organization": ["成员需要使用已有效链接 SSO 的账户来访问该组织"],
    "Enable SCIM": ["启用SCIM"],
    "Enable SCIM to manage Memberships and Teams via your Provider": ["使 SCIM 能够通过提供商管理成员资格和团队"],
    "Default Role": ["默认角色"],
    "The default role new members will receive when logging in for the first time.": ["新成员第一次登录时将接到的默认角色。"],
    ...
    "%s@%s": ["%s@%s"],
    "You are viewing a frontend deploy preview of [commitLink] ([branchLink])": ["您正在查看 [commitLink] ([branchLink]) 的前端部署视图"],
    "You are developing against production Sentry API, please BE CAREFUL, as your changes will affect production data.": ["您正在使用生产Sentry API进行开发，请务必小心，因为您的改动将会影响线上数据"],
    ...
    "Changed [fieldName] from [oldValue] to [newValue]": ["修改[fieldName]：[oldValue] 至 [newValue]"],
    "Unable to restore [fieldName] from [oldValue] to [newValue]": ["无法恢复 [fieldName]：[oldValue] 至 [newValue]"],
    "Restored [fieldName] from [oldValue] to [newValue]": ["恢复 [fieldName]：[oldValue] 至 [newValue]"],
    "Saving changes...": ["保存修改"],
    ...
    "Enabling...": ["启用中..."],
    "A request was sent to move [project] to a different organization": ["一个请求已经被发送用于转移项目 [project] 到另一个组织"],
    "Error transferring [project]. [message]": ["出现传输错误 [project]. [message]"],
    "Error transferring [project]": ["转移 [project] 时出错"],
    "[team] has been added to the [project] project": ["[team] 已经被添加至 [project] 项目"],
    "Unable to add [team] to the [project] project": ["未能添加 [team] 到 [project] 项目"],
    "[team] has been removed from the [project] project": ["[team] 已经被从 [project] 项目中移除"],
    "Unable to remove [team] from the [project] project": ["未能从 [project] 项目中移除 [team]"],
    ...
    "You have too many unique tags and some have been truncated": ["你有太多的不同标签了，一些已经被缩减"],
    "Unable to remove [team] from the [organization] organization": ["未能从 [organization] 组织中移除 [team]"],
    "[team] has been added to the [organization] organization": ["[team] 已经被添加到 [organization] 组织"],
    "Unable to create [team] in the [organization] organization": ["未能在 [organization] 组织中创建 [team]"],
    "[team] has been removed from the [organization] organization": ["[team] 已经从 [organization] 组织中移除"],
    "This feature is coming soon!": ["这个特性即将到来！"],
    "Enables the %s feature": ["开启%s特性"],
    "This feature is not enabled on your Sentry installation.": ["这个特性在您的Sentry服务上没有开启。"],
    ...
    "Ignore": ["忽略"],
    "Unignore": ["取消忽略"],
    "Silences alerts for this issue and removes it from the issue stream by default.": ["默认情况下，将此问题的警报静音，并将其从问题流中删除。"],
    "per hour": ["1小时"],
    "per day": ["每天"],
    "per week": ["每周"],
    "Ignore this issue until it occurs again…": ["忽略该问题直到其再次复现..."],
    "Number of times": ["次数"],
    "Ignore this issue until it affects an additional…": ["忽略这个问题直到它影响到其他..."],
    ...
    "Unresolve": ["未解决"],
    "This event is resolved due to the Auto Resolve configuration for this project": ["该事件已通过本项目的自动解决配置解决"],
    "Set up release tracking in order to use this feature.": ["设置发布跟踪以使用这个新特性"],
    ...
    "Resolve": ["解决"],
    "Edit": ["编辑"],
    "You can edit this comment due to your superuser status": ["由于您是超级用户所以您可以编辑该评论"],
    "Are you sure you wish to delete this comment?": ["确定要删除该评论？"],
    "You can delete this comment due to your superuser status": ["由于您的超级用户状态，您可以删除此评论"],
    ...
    "You can auto-assign issues by adding [issueOwners:Issue Owner rules].": ["您可以通过添加 [问题所有者：问题所有者规则] 来自动分配问题。"],
    ...
    "Gauge performance using different metrics for each project. Set response time thresholds, per project, for the Apdex and User Misery Scores in each project’s Performance settings.": ["为每个项目使用不同的指标来衡量绩效。在每个项目的性能设置中为每个项目的 Apdex 和用户痛苦分数设置响应时间阈值。"],
    "Response Time Thresholds": ["响应时间阈值"],
    "Use this menu to adjust each transaction’s satisfactory response time threshold, which can vary across transactions. These thresholds are used to calculate Apdex and User Misery, metrics that indicate how satisfied and miserable users are, respectively.": ["使用此菜单可以调整每个事务的满意响应时间阈值，该阈值可能因事务而异。这些阈值用于计算Apdex和用户痛苦，这两个指标分别表示用户的满意度和痛苦程度。"],
    "Grouping": ["归组"],
    "Projects": ["项目"],
    "Create a project for any type of application you want to monitor.": ["为需要监控的任何类型应用创建项目"],
    "Issues": ["问题"],
    "Performance": ["性能"],
    "Releases": ["版本"],
    "Track the health of every release, see differences between releases from crash analytics to adoption rates.": ["跟踪每个版本的运行状况，查看版本之间从崩溃分析到采用率的差异。"],
    "Discover": ["发现"],
    "Query and unlock insights into the health of your entire system and get answers to critical business questions all in one place.": ["查询并解锁对整个系统运行状况的见解，并在一个位置获得关键业务问题的答案。"],
    "Retrace Your Steps": ["追溯您的操作步骤"],
    "Annoy the Right People": ["惹恼合适的人"],
    "Today only admins in your organization can create alert rules but we recommend [link:allowing members to create alerts], too.": ["目前，只有组织中的管理员才能创建警报规则，但我们也建议 [链接：允许成员创建警报]。"],
    "Allow": ["允许"],
    "Event Breakdown": ["事件细分"],
    "The event breakdown shows you the breakdown of event types within a trace.": ["事件细分显示跟踪中事件类型的细分。"],
    "Filter by Span Operation": ["按跨度操作筛选"],
    "You can now filter these transaction events based on http, db, browser or resource operation.": ["您现在可以根据 http、db、浏览器或资源操作过滤这些事务事件。"],
    "Suspect Tags": ["可疑标签"],
    "See which tags often correspond to slower transactions. You'll want to investigate these more. [link:Learn more]": ["查看哪些标签通常对应于较慢的事务。您需要进一步调查这些内容。[友情链接：了解更多]"],
    "Dismiss": ["忽略"],
    "Enough Already": ["已经足够"],
    "Got It": ["知道了"],
    ...
    "Save Avatar": ["保存头像"],
    "Please upload an image larger than [size]px by [size]px.": ["请上传一张大于 [size]px x [size]px 的图片。"],
    "Please upload an image smaller than [size]px by [size]px.": ["请上传小于 [size]px x x [size]px 的图片。"],
    ...
    "This feature is internal and available for QA purposes": ["此功能是内置的，可用于 QA 目的"],
    "This feature is available for early adopters and may change": ["此功能适用于早期采用者，可能会更改"],
    "This feature is new! Try it out and let us know what you think": ["此功能是新功能！尝试一下，让我们知道您的想法"],
    "alpha": ["阿尔法"],
    "beta": ["内测"],
    "new": ["新增功能"],
    "Learn more": ["了解更多"],
    ...
    "Previous": ["上一个"],
    "previous %s": ["前 %s"],
    "%s has an invalid date range. Please try a more recent date range.": ["%s 的日期范围无效。请尝试最近的日期范围。"],
    ...
    "Loading...": ["加载中..."],
    "Unknown Commit": ["未知提交"],
    "The email [actorEmail] is not a member of your organization. [inviteUser:Invite] them or link additional emails in [accountSettings:account settings].": ["电子邮箱 [actorEmail] 不是组织的成员。[邀请用户：邀请]他们或在[帐户设置：帐户设置]中链接其他电子邮箱。"],
    "Unknown author": ["未知作者"],
    ...
    "My Projects": ["我的项目"],
    "All Projects": ["全部项目"],
    "You have no projects. Click [link] to make one.": ["你没有项目。 点击[link]以新建一个。"],
    "here": ["这里"],
    ...
    "Ask your organization owner or manager to [settingsLink:enable alerts access] for you.": ["请您的组织所有者或管理员为您 [设置链接：启用警报访问权限]。"],
    "Create Alert": ["创建警报"],
    "Created": ["创建"],
    "e.g. 100": ["例如：100"],
    "Time window": ["时间窗口"],
    "e.g. per hour": ["例如：每小时"],
    "(Optional) If supplied, this rule will apply as a rate of change.": ["（可选的）如果提供，此规则将作为更改率应用"],
    "Ignore this issue until …": ["忽略该问题直到 ..."],
    "Time (UTC)": ["时间（UTC）"],
    "Please enter a valid date in the future": ["请输入一个未来的日期"],
    "e.g. 1.0.4": ["例如：1.0.4"],
    "Version": ["版本"],
    "Sit tight. We'll shoot you an email when your data is ready for download.": ["耐心等待。当您的数据可供下载时，我们会向您发送一封电子邮箱。"],
    "It looks like we're already working on it. Sit tight, we'll email you.": ["看起来我们已经在努力了。请坐，我们会给您发电子邮箱。"],
    ...
    "Unable to save your changes. Please correct the following errors try again.": ["无法保存你的修改。请更正下面的错误后重试。"],
    "Unable to save your changes. Please ensure all fields are valid and try again.": ["无法保存你的修改。确保所有字段都正确，请重试。"],
    ...
    "For more performance metrics, specify which platform this project is using in [link]": ["有关更多性能指标，请在 [link] 中指定此项目正在使用的平台"],
    ...
    "Span durations are summed over the course of an entire transaction. Any overlapping spans are only counted once.": ["跨度持续时间是在整个事务过程中求和的。任何重叠跨度仅计数一次。"],
    "No items": ["没有元素"],
    "Filter search": ["过滤搜索"],
    "found": ["发现"],
    "Searching…": ["搜索。。。"],
    "Something went horribly wrong rendering this page.\nWe use a decent error reporting service so this will probably be fixed soon. Unless our error reporting service is also broken. That would be awkward.\nAnyway, we apologize for the inconvenience.": ["渲染页面时出现了一些错误，我们使用了一个不错的错误报告服务，这个问题可能将很快被修复，除非我们的错误报告服务也损坏了。那将十分令人尴尬。总而言之，我们为带来的不变而道歉。"],
    ...
    "There is an internal systems error or active issue": ["内部系统错误或活动的问题"],
    "This could be due to a handful of reasons:": ["这个可能有以下几个原因："],
    "No events were found for the currently selected environments": ["当前所选环境未找到事件"],
    "Sorry, the events for this issue could not be found.": ["对不起，这个问题的事件找不到了"],
    "If all else fails, [link:contact us] with more details": ["如果所有其他方法都失败了，[链接：联系我们]提供更多细节"],
    "Not sure what to do? [link:Return to the dashboard]": ["不确定接下来该做什么？ [link:返回仪表盘页面]"],
    "in [location]": ["在 [位置]"],
    "This is an attachment containing a session replay. [replayLink:View the replay] or [jsonLink:view the raw JSON].": ["这是包含会话重播的附件。[重播链接：查看重播]或[json链接：查看原始JSON]。"],
    "Start Over": ["重新开始"],
    "Try Again": ["重试"],
    "Send": ["发送"],
    "Later": ["之后"],
    ...
    "Attachments": ["附件"],
    "Attachments (%s)": ["附件 (%s)"],
    "Your limit of stored crash reports has been reached for this issue. [attachmentsLink: View crashes] or [settingsLink: configure limit].": ["已达到此问题的存储崩溃报告限制。[附件链接：视图崩溃]或[设置链接：配置限制]。"],
    ...
    "Date Created": ["创建日期"],
    "MIME Type": ["哑剧类型"],
    "Screenshots help identify what the user saw when the event happened": ["屏幕截图有助于识别事件发生时用户看到的内容"],
    "Are you sure you wish to delete this screenshot?": ["您确定要删除此屏幕截图吗？"],
    "Tags": ["标签"],
    "Fails threshold at %s.": ["在 %s 处失败阈值。"],
    "These vitals were collected using an outdated SDK version and may not be accurate. To ensure accurate web vitals in new transaction events, please update your SDK to the latest version.": ["这些指标是使用过时的 SDK 版本收集的，可能不准确。为确保新交易事件中的 Web 指标准确无误，请将您的 SDK 更新到最新版本。"],
    ...
    "The server-side fingerprinting rule that produced the fingerprint.": ["生成指纹的服务器端指纹规则。"],
    "Fingerprint values": ["指纹值"],
    "Client fingerprint values": ["客户端指纹值"],
    "The client sent a fingerprint that was overridden by a server-side fingerprinting rule.": ["客户端发送的指纹已被服务器端指纹规则覆盖。"],
    "Hash": ["Hash"],
    "Events with the same hash are grouped together": ["hash值相同的事件被归为一组"],
    "Hash mismatch": ["Hash值不匹配"],
    "hashing algorithm produced a hash that does not match the event": ["哈希算法生成与事件不匹配的哈希"],
    "Uses a complex grouping algorithm taking event data into account": ["使用考虑事件数据的复杂分组算法"],
    "Grouping Config": ["归组配置"],
    "Overrides the default grouping by a custom fingerprinting rule": ["通过自定义指纹规则覆盖默认分组"],
    "Uses a complex grouping algorithm taking event data and a fingerprint into account": ["使用复杂的分组算法，考虑事件数据和指纹"],
    ...
    "Time": ["时间"],
    "Sorry, no breadcrumbs match your search query": ["抱歉，没有与您的搜索查询匹配的痕迹导航"],
    "Reset filter": ["重置过滤器"],
    ...
    "Some frames appear to be minified. Did you configure the [plugin]?": ["某些帧似乎已缩小。您是否配置了 [插件]？"],
    ...
    "Frames %d until %d were omitted and not available.": ["栈帧 %d 到 %d 将会被忽略并且不可用。"],
    "Report": ["报告"],
    "CSP Report": ["CSP 报告"],
    "The debug information file for this image could not be downloaded": ["无法下载此映像的调试信息文件"],
    "The debug information file for this image failed to process": ["无法处理此映像的调试信息文件"],
    "No debug information could be found in any of the specified sources": ["在任何指定的源中都找不到调试信息"],
    "Debug information for this image was found and successfully processed": ["找到并成功处理此映像的调试信息"],
    "The image was not required for processing the stack trace": ["处理堆栈跟踪不需要该映像"],
    ...
    "Actions not available because this debug file was deleted": ["操作不可用，因为此调试文件已删除"],
    "This debug file was uploaded [when] after this event.": ["此调试文件是[在]此事件之后上传的。"],
    "Uploaded [timesince]": ["上传 [时间自]"],
    "This debug file was uploaded [when] before this event. It takes up to 1 hour for new files to propagate. To apply new debug information, reprocess this issue.": ["此调试文件是[在]此事件之前上传的。新文件最多需要 1 小时才能传播。若要应用新的调试信息，请重新处理此问题。"],
    "This debug file was uploaded [when] before this event. It takes up to 1 hour for new files to propagate.": ["此调试文件是[在]此事件之前上传的。新文件最多需要 1 小时才能传播。"],
    "This debug file was uploaded [when] after this event. To apply new debug information, reprocess this issue.": ["此调试文件是[在]此事件之后上传的。若要应用新的调试信息，请重新处理此问题。"],
    "Failed": ["失败"],
    "Not Found": ["未找到"],
    "Permissions": ["权限"],
    "Unapplied": ["未应用"],
    "debug": ["调试"],
    "Debug information provides function names and resolves inlined frames during symbolication": ["调试信息在符号化期间提供函数名称并解析内联帧"],
    "sources": ["来源"],
    "Source code information allows Sentry to display source code context for stack frames": ["源代码信息允许Sentry显示堆栈帧的源代码上下文"],
    "symtab": ["符号选项卡"],
    "Symbol tables are used as a fallback when full debug information is not available": ["当调试信息文件不可用时，符号表将作为备用被使用。"],
    "unwind": ["放松"],
    "Stack unwinding information improves the quality of stack traces extracted from minidumps": ["堆栈展开信息提高了从小型转储中提取的堆栈跟踪的质量"],
    "This debug information file is from Sentry's internal symbol server for this project": ["此调试信息文件来自此项目的 Sentry 内部符号服务器"],
    "This debug information file is from a built-in symbol server": ["此调试信息文件来自内置符号服务器"],
    "This debug information file is from a custom symbol server": ["此调试信息文件来自自定义符号服务器"],
    "Download Details": ["下载详情"],
    "Permission Error": ["权限错误"],
    "This file was deleted after the issue was processed.": ["处理问题后，此文件已删除。"],
    "This issue was processed before this debug information file was available. To apply new debug information, reprocess this issue.": ["在此调试信息文件可用之前，已处理此问题。若要应用新的调试信息，请重新处理此问题。"],
    "This issue was processed before this debug information file was available": ["在此调试信息文件可用之前，已处理此问题"],
    "Sorry, no debug files match your search query": ["抱歉，没有与您的搜索查询匹配的调试文件"],
    "There are no debug files to be displayed": ["没有要显示的调试文件"],
    "Debug File Candidates": ["调试文件候选项"],
    "These are the Debug Information Files (DIFs) corresponding to this image which have been looked up on [docLink:symbol servers] during the processing of the stacktrace.": ["这些是与此映像对应的调试信息文件 （DIF），在处理堆栈跟踪期间已在 [docLink：符号服务器] 上查找。"],
    "Source": ["源"],
    ...
    "Image": ["图像"],
    "Search for this debug file in all images for the %s project": ["在%s项目的所有图像中搜索此调试文件"],
    "Open in Settings": ["在“设置”中打开"],
    "You’ve uploaded new debug files. Reprocess events in this issue to view a better stack trace": ["您已上传新的调试文件。重新处理此问题中的事件以查看更好的堆栈跟踪"],
    "This event cannot be reprocessed because the event has not been found": ["无法重新处理此事件，因为尚未找到该事件"],
    "This event cannot be reprocessed because a required attachment is missing": ["无法重新处理此事件，因为缺少所需的附件"],
    ...
    "Hexadecimal": ["十六进制"],
    "Numeric": ["数值"],
    "This frame appears in all other events related to this issue": ["此帧出现在与此问题相关的所有其他事件中"],
    ...
    "No function name was supplied by the client SDK.": ["客户端 SDK 未提供函数名称。"],
    ...
    "The debug file could not be retrieved from any of the sources.": ["无法从任何源检索调试文件。"],
    "The retrieved debug file could not be processed.": ["无法处理检索到的调试文件。"],
    "Unknown system frame. Usually from beta SDKs": ["未知的系统框架，通常源于测试版SDK。"],
    "Called from": ["从 调用"],
    "Message": ["消息"],
    ...
    "For in-depth instructions on setting up tracing, view [docLink:our documentation].": ["有关设置跟踪的深入说明，请查看 [docLink：我们的文档]。"],
    "To manually instrument certain regions of your code, view [docLink:our documentation].": ["要手动检测代码的某些区域，请查看 [docLink：我们的文档]。"],
    ...
    "Regroup": ["重组"],
    "This span is showing a direct child. Remove transaction to hide": ["此跨度显示的是直接子级。删除要隐藏的交易"],
    "This span has a direct child. Add transaction to view": ["此跨度具有直接子级。添加交易记录以查看"],
    "Loading embedded transaction": ["加载嵌入式事务"],
    "Error loading embedded transaction": ["加载嵌入式事务时出错"],
    "View Children": ["查看儿童"],
    "View Summary": ["查看摘要"],
    "This is a span that has no parent span within this transaction. It has been attached to the transaction root span by default.": ["这是此事务中没有父跨度的跨度。默认情况下，它已附加到事务根跨度。"],
    ...
    "The next spans are unavailable. You may have exceeded the span limit or need to address missing instrumentation.": ["下一个跨度不可用。您可能已超出跨度限制或需要解决缺少的检测问题。"],
    "Autogrouped ": ["自动分组"],
    " and more": ["等等"],
    "View Trace": ["查看跟踪"],
    "Show less": ["显示更少"],
    ...
    "Span durations are summed over the course of an entire transaction. Any overlapping spans are only counted once. Percentages are calculated by dividing the summed span durations by the total of all span durations.": ["跨度持续时间是在整个事务过程中求和的。任何重叠跨度仅计数一次。百分比的计算方法是将总的跨度持续时间除以所有跨度持续时间的总和。"],
    "Assemblies": ["组件"],
    "Packages": ["软件包"],
    ...
    "Event Processing for this project is currently degraded. Events may appear with larger delays than usual or get dropped. Please check the [link:Status] page for a potential outage.": ["此项目的事件处理当前已降级。事件可能会比平时出现更大的延迟或被丢弃。请查看 [链接：状态] 页面以了解潜在的中断。"],
    "Event Processing for the [projectSlugs] projects is currently degraded. Events may appear with larger delays than usual or get dropped. Please check the [link:Status] page for a potential outage.": ["[projectSlugs] 项目的事件处理当前已降级。事件可能会比平时出现更大的延迟或被丢弃。请查看 [链接：状态] 页面以了解潜在的中断。"],
    "Close Modal": ["关闭对话框"],
    ...
    "When the most recent event in this issue was captured.": ["捕获本期中的最新事件的时间。"],
    "When the first event in this issue was captured.": ["捕获本期中的第一个事件的时间。"],
    ...
    "There don't seem to be any events fitting the query.": ["似乎没有任何适合查询条件的事件。"],
    "Graph": ["图"],
    "events": ["事件"],
    "Priority": ["优先级"],
    "Unknown Author": ["未知作者"],
    ...
    "Max widgets ([maxWidgets]) per dashboard reached.": ["达到每个仪表板的最大小组件（[maxWidgets]）。"],
    "Internal Integrations": ["内部集成"],
    "Public Integrations": ["公开集成"],
    "Create Team": ["创建团队"],
    "Search events": ["搜索事件"],
    "Multiple queries were used to create this widget visualization. Which query would you like to view in Discover?": ["多个查询用于创建此小组件可视化。您希望在“发现”中查看哪个查询？"],
    "Download Url": ["下载地址"],
    ...
    "Bucket": ["桶"],
    "Name of the GCS bucket. Read permissions are required to download symbols.": ["GCS bucket名，必须具有读权限以下载数据。"],
    "Client Email": ["客户电子邮箱"],
    "Email address of the GCS service account.": ["GCS 服务帐户的电子邮件地址。"],
    "Private Key": ["私钥"],
    "(Private Key unchanged)": ["（私有键不变）"],
    "The service account key. Credentials can be managed on the [link].": ["服务帐户密钥。可以在 [链接] 上管理凭据。"],
    "The path at which files are located within this repository.": ["文件在此存储库中所在的路径。"],
    "Name of the S3 bucket. Read permissions are required to download symbols.": ["S3 存储桶的名称。下载符号需要读取权限。"],
    "Region": ["地区"],
    "The AWS region and availability zone of the bucket.": ["存储桶的 AWS 区域和可用区。"],
    "Access Key ID": ["Access Key ID"],
    "Access key to the AWS account. Credentials can be managed in the [link].": ["AWS账号的Access Key， 该认证信息可在此页面管理： [link]。"],
    "Edit Ownership Rules": ["编辑所有权规则"],
    "Action Required": ["所需操作"],
    "Please verify your email before [actionMessage], or [link].": ["请在 [操作消息] 或 [链接] 之前验证您的电子邮箱。"],
    "go to your email settings": ["转到您的电子邮箱设置"],
    "%s of %s": ["%s 的 %s"],
    "Search for documentation, FAQs, blog posts...": ["搜索文档、常见问题解答、博客文章..."],
    ...
    "Invite new members by email to join your organization.": ["通过电子邮箱邀请新成员加入您的组织。"],
    ...
    "Sent [invites], [failed] failed to send.": ["已发送 [invites], [failed] 发送失败."],
    "Sent [invites]": ["已发送 [invites]"],
    "%s invite requests": [
      "%s 邀请请求",
    ],
    "[inviteRequests] pending approval, [failed] failed to send.": ["[邀请请求] 等待批准，[失败] 发送失败。"],
    ...
    "Two-factor authentication via %s has been enabled.": ["通过 %s 的双重认证已经启用。"],
    "You should now set up recovery options to secure your account.": ["现在，您应该设置恢复选项以保护您的账户安全。"],
    "We recommend adding a phone number as a backup 2FA method.": ["我们建议添加一个手机号码作为备份的2FA方式。"],
    "Recovery codes are the only way to access your account if you lose\n                  your device and cannot receive two-factor authentication codes.": ["`恢复代码` 用于您丢失了设备并无法使用二步验证时进行访问"],
    ...
    "You will be redirected to the new project [project] in [timer] seconds...": ["在[timer]秒内，您将被跳转至新项目[project]..."],
    "Continue to %s": ["继续%s"],
    "Reprocess Events": ["重新处理事件"],
    "Reprocessing applies new debug files and grouping enhancements to this Issue. Please consider these impacts:": ["重新处理会对此问题应用新的调试文件和分组增强功能。请考虑以下影响："],
    "[strong:Quota applies.] Every event you choose to reprocess counts against your plan's quota. Rate limits and spike protection do not apply.": ["[强：配额适用。] 您选择重新处理的每个事件都会占用您的计划配额。速率限制和尖峰保护不适用。"],
    "[strong:Attachment storage required.] If your events come from minidumps or unreal crash reports, you must have [link:attachment storage] enabled.": ["[强：需要附件存储。] 如果事件来自小型转储或虚幻崩溃报告，则必须启用 [链接：附件存储]。"],
    "Please wait one hour after upload before attempting to reprocess missing debug files.": ["请在上传后等待一小时，然后再尝试重新处理丢失的调试文件。"],
    "For more information, please refer to [link:the documentation.]": ["有关详细信息，请参阅 [链接：文档。]"],
    "Failed to reprocess. Please check your input.": ["无法重新处理。请检查您的输入。"],
    "Number of events to be reprocessed": ["要重新处理的事件数"],
    "If you set a limit, we will reprocess your most recent events.": ["如果您设置了限制，我们将重新处理您最近的事件。"],
    "Reprocess all events": ["重新处理所有事件"],
    "Remaining events": ["剩余事件"],
    "What to do with the events that are not reprocessed.": ["如何处理未重新处理的事件。"],
    "Keep": ["保持"],
    "Saving Changes": ["保存更改"],
    "Unable to save your changes.": ["保存变更失败"],
    "[read] and [write] access to [resources] resources": ["对 [资源] 资源的 [读取] 和 [写入] 访问权限"],
    ...
    "You are attempting to access a resource that requires superuser access, please re-authenticate as a superuser.": ["您正在尝试访问需要超级用户访问权限的资源，请以超级用户身份重新认证。"],
    "Confirm Password to Continue": ["认证密码以继续"],
    "Team request sent for approval": ["团队请求已发送以供审批"],
    "Unable to send team request": ["无法发起加入团队请求"],
    "You do not have permission to add members to the #[team] team, but we will send a request to your organization admins for approval.": ["您没有权限直接为#[team] 团队添加成员，但我们将会将该添加请求发送给组织管理员审批。"],
    "All Events": ["所有事件"],
    "Edit Widget": ["编辑小部件"],
    ...
    "Some tasks should be completed before completing these tasks": ["有些任务应在完成这些任务之前完成"],
    "Completed": ["完成"],
    "Community Forum": ["社区论坛"],
    "Not sure what to do? We're here for you!": ["不知道该怎么办？ 我们在这里为您服务！"],
    "Just skip": ["直接跳过"],
    ...
    "Collect user feedback when your application crashes": ["当你的应用崩溃时收集用户的反馈"],
    "Set up issue tracking": ["设置问题跟踪"],
    "Link to Sentry issues within your issue tracker": ["在你的问题跟踪器中链接到 Sentry 问题页面"],
    ...
    "Sending test...": ["发送测试..."],
    "Test Complete!": ["测试完成！"],
    "An unexpected error occurred while testing your plugin. Please try again.": ["当测试您的插件时发生了一个错误，请重试。"],
    "Test Plugin": ["测试插件"],
    "This plugin is considered beta and may change in the future.": ["此插件被认为是测试版，将来可能会更改。"],
    "Oops! Looks like there aren't any available integrations installed.": ["Oops！看起来似乎没有任何可用集合被安装。"],
    "This feature is a preview and may change in the future. Thanks for being an early adopter!": ["这个功能是个预览版，未来有可能发生改变，感谢成为早期采纳者。"],
    "Samples": ["例子"],
    "OS": ["操作系统"],
    "Count": ["计数"],
    "Platform": ["平台"],
    ...
    "Request Access": ["请求访问"],
    "No teams have access to this project yet. Ask an admin to add your team to this project.": ["这个项目还没有团队加入，请管理员将你的团队加入这个项目"],
    ...
    "Settings": ["设置"],
    "Pause": ["暂停"],
    "You must be an organization owner, manager or admin to remove a repository.": ["您必须是组织所有者、经理或管理员才能删除仓库。"],
    "Are you sure you want to remove this repository? All associated commit data will be removed in addition to the repository.": ["您确定要删除此仓库吗？ 除了仓库之外，所有与该仓库关联的提交数据也都将被删除。"],
    "[actor] marked this issue as resolved in the upcoming release.": ["[actor] 将这个问题在即将到来的版本中标记为已解决。"],
    "This issue has been marked as resolved in the upcoming release.": ["这个问题在即将到来的版本中标记为已解决。"],
    "[actor] marked this issue as resolved in version [version].": ["[actor] 将这个问题在版本[version]中标记为已解决。"],
    "This issue has been marked as resolved in version [version].": ["这个问题在版本[version]中被标记为已解决。"],
    "This issue has been marked as resolved by [commit]": ["这个问题被标记为已解决。"],
    "This issue has been marked as resolved.": ["这个问题被标记为已解决。"],
    "Unable to open search result (a popup blocker may have caused this).": ["无法打开搜索结果（可能是弹出窗口阻止程序导致的）。"],
    "Event": ["事件"],
    "%s Dashboard": ["%s 仪表盘"],
    ...
    "May contain lowercase letters, numbers, dashes and underscores.": ["可以包含小写字母，数字，点和下划线"],
    "You do not have permission to add team to project.": ["您没有向项目中添加团队的权限。"],
    ...
    "Insert your U2F device or tap the button on it to confirm the sign-in request.": ["插入您的UrF设备或点击按钮来认证登录请求。"],
    "Alternatively you can use your U2F device to confirm the action.": ["另外你可以使用你的U2F设备认证动作。"],
    "To enroll your U2F device insert it now or tap the button on it to activate it.": ["要注册您的U2F设备，请立即将其插入或点击按钮将其激活。"],
    "New Issues": ["新问题"],
    "commits ": ["提交"],
    "commit ": ["提交"],
    "by ": ["由 "],
    ...
    "Enable this to receive notifications about your own actions on Sentry.": ["开启以接收关于你自己在Sentry上动作的通知。"],
    "Claim Unassigned Issues I've Resolved": ["认领我已解决的未分配问题"],
    "You'll receive notifications about any changes that happen afterwards.": ["您将收到后续任何变化的通知。"],
    "Theme": ["主题"],
    "Language": ["语言"],
    "Timezone": ["时区"],
    "Use a 24-hour clock": ["使用 24 小时制"],
    "Use default ignored sources": ["使用默认忽略源"],
    "Our default list will attempt to ignore common issues and reduce noise.": ["我们的默认列表会尝试忽略常见的问题。"],
    "Additional ignored sources": ["附加忽略源"],
    "Discard reports about requests from the given sources. Separate multiple entries with a newline.": ["当清洗所有项目的数据时需要附加字段名称匹配，用换行符分割多个条目。"],
    ...
    "You will be redirected to the new organization slug after saving": ["在保存后您将被跳转到新的组织slug"],
    "Display Name": ["显示名称"],
    "Early Adopter": ["早期使用者"],
    "The default role new members will receive": ["新成员将会收到的默认角色"],
    "Open Membership": ["开放式群组"],
    "Allow members to delete events (including the delete & discard action) by granting them the `event:admin` scope.": ["通过授予成员`event:admin`来允许他们删除事件（包括删除和丢弃操作）。"],
    "Disabling this can have privacy implications for ALL projects, are you sure you want to continue?": ["禁用次选项可能对全部项目都有影响，您确定要继续吗？"],
    "Global Sensitive Fields": ["全局敏感字段"],
    "Additional field names to match against when scrubbing data for all projects. Separate multiple entries with a newline.": ["当清洗所有项目的数据时需要附加字段名称匹配，用换行符分割多个条目。"],
    "Note: These fields will be used in addition to project specific fields.": ["注意：除了项目特定字段外，还将使用这些字段。"],
    "e.g. business-email": ["例如：业务邮箱"],
    "Field names which data scrubbers should ignore. Separate multiple entries with a newline.": ["数据清洗器忽略的字段。多个字段以换行符分隔。"],
    "Note: These fields will be used in addition to project specific fields": ["注意：除了项目特定字段外，还将使用这些字段"],
    "Require Two-Factor Authentication": ["要求双重认证"],
    "Prevent Storing of IP Addresses": ["避免存储 IP 地址"],
    "Preventing IP addresses from being stored for new events on all projects": ["避免全部项目的新事件中包含的IP地址被存储。"],
    "This will remove all members without two-factor authentication from your organization. It will also send them an email to setup 2FA and reinstate their access and settings. Do you want to continue?": ["这将会立刻强制全部用户启用双重认证。也将发送一封邮件提醒设置双重认证。您确定要继续吗？"],
    "Are you sure you want to allow users to access your organization without having two-factor authentication enabled?": ["您确定您希望允许用户访问您的组织而不需要开启双重认证吗？"],
    "Allow Shared Issues": ["允许共享问题"],
    "Enable sharing of limited details on issues to anonymous users": ["开启对匿名用户分享有限问题细节"],
    "Are you sure you want to allow sharing issues to anonymous users?": ["您确定您允许对匿名用户分享问题？"],
    "Enhanced Privacy": ["增强隐私"],
    "Enable enhanced privacy controls to limit personally identifiable information (PII) as well as source code in things like notifications": ["启用增强的隐私控制以限制在通知之类东西里的个人身份信息（PII）和源代码"],
    "Allow Sentry to scrape missing JavaScript source context when possible": ["允许Sentry 尽可能抓取丢失的 JavaScript 源代码"],
    "Store Native Crash Reports": ["存储原生崩溃报告"],
    "Allow Join Requests": ["允许加入请求"],
    "Require Data Scrubber": ["需要数据清洗"],
    "Require server-side data scrubbing be enabled for all projects": ["要求为全部项目启用服务端侧数据清洗"],
    "Security & Privacy": ["安全和隐私"],
    "Enable server-side data scrubbing": ["启用服务端数据清洗"],
    "Require Using Default Scrubbers": ["要求使用默认的数据清洗器"],
    "Require the default scrubbers be applied to prevent things like passwords and credit cards from being stored for all projects": ["要求为所有项目应用默认清洗器以防止密码和信用卡之类的信息被存储"],
    "Subject Template": ["题目模板"],
    "Minimum delivery interval": ["最小发送间隔"],
    "Notifications will be delivered at most this often.": ["通知最多会发送这么多次。"],
    "Maximum delivery interval": ["最大发送间隔"],
    "Notifications will be delivered at least this often.": ["通知至少会发送这么多次。"],
    "%s minutes": [
      "%s 分钟",
    ],
    "%s days": [
    ...
    "Security Token": ["安全令牌"],
    "Outbound requests matching Allowed Domains will have the header \"{token_header}: {token}\" appended": ["匹配允许域名的外网请求需要附有\"{token_header}: {token}\""],
    "X-Sentry-Token": ["X-Sentry-Token"],
    "Security Token Header": ["安全令牌头"],
    "Verify TLS/SSL": ["验证TLS/SSL"],
    "Outbound requests will verify TLS (sometimes known as SSL) connections": ["外网请求要验证TSL（SSL）链接"],
    "This option is enforced by your organization's settings and cannot be customized per-project.": ["这个选项已经在你的组织中设置成了强制项，不能给每个项目另外设置。"],
    "Subject Prefix": ["主题前缀"],
    "e.g. [my-org]": ["例如：[我的组织]"],
    "Choose a custom prefix for emails from this project": ["为这个项目的邮件选择一个自定义的前缀"],
    "Auto Resolve": ["自动解决"],
    "Automatically resolve an issue if it hasn't been seen for this amount of time": ["自动将一段时间内未复现的事件标记为解决"],
    "business-email": ["业务邮箱"],
    "Safe Fields": ["安全字段"],
    "Field names which data scrubbers should ignore. Separate multiple entries with a newline": ["数据清洗器忽略的字段。多个字段以换行符分隔。"],
    "Data Scrubber": ["数据清洗"],
    "Are you sure you want to disable server-side data scrubbing?": ["您确定您要禁用服务端数据清洗？"],
    "Use Default Scrubbers": ["使用默认数据清洗器"],
    "Apply default scrubbers to prevent things like passwords and credit cards from being stored": ["应用默认清洗器以防止密码和信用卡之类的信息被存储"],
    "Are you sure you want to disable using default scrubbers?": ["您确定要禁用默认清洗器？"],
    "Preventing IP addresses from being stored for new events": ["防止新事件的IP地址被存储"],
    "Are you sure you want to disable scrubbing IP addresses?": ["您确定要禁用IP地址清洗？"],
    "email": ["电子邮件"],
    "Additional Sensitive Fields": ["其他敏感字段"],
    "Additional field names to match against when scrubbing data. Separate multiple entries with a newline": ["当清洗所有项目的数据时需要附加字段名称匹配，用换行符分割多个条目。"],
    "Team Slug": ["团队Slug"],
    "A unique ID used to identify the team": ["一个用于验证团队的唯一ID"],
    "You will be redirected to the new team slug after saving": ["保存后您将被跳转到新的团队slug"],
    "Popular": ["常见的"],
    "Manual": ["手动"],
    ...
    "An unknown error occurred. Need help with this? [link:Contact support]": ["发生了未知错误。是否需要帮助？[link:Contact support]"],
    ...
    "Please select which [organization] you want for the project [project].": ["请选择您希望将项目[project]转移到哪一个[organization]。"],
    "Server Version": ["服务器版本"],
    "Python Version": ["Python版本"],
    "Configuration File": ["配置文件"],
    "Uptime": ["运行时间"],
    "Environment not found (are you using the builtin Sentry webserver?).": ["找不到环境变量(您是在使用内置的Sentry web服务器吗？)"],
    ...
    "Test Settings": ["测试设置"],
    "Send an email to your account's email address to confirm that everything is configured correctly.": ["向您的电子邮件地址发送一封邮件以确认配置正常。"],
    "Organizations": ["组织"],
    "Accepted": ["已接受"],
    "Dropped": ["减少"],
    ...
    "Remove User": ["删除用户"],
    "The username is the unique id of the user in the system": ["用户名是用户在系统中的唯一标识"],
    "The users primary email address": ["用户的主电子邮件地址"],
    "Designates whether this user can perform administrative functions.": ["是否允许该用户进行管理操作。"],
    "Superuser": ["超级用户"],
    "Designates whether this user has all permissions without explicitly assigning them.": ["是否允许该用户拥有所有未特别分配给他的权限。"],
    "Disable User": ["禁用用户"],
    ...
    "Complete setup by filling out the required configuration.": ["填写必须的配置以完成安装。"],
    "Setup Sentry": ["安装Sentry"],
    "Welcome to Sentry": ["欢迎来到Sentry"],
    "We were unable to load the required configuration from the Sentry server. Please take a look at the service logs.": ["无法从 Sentry 服务器读取必须的配置。请查看日志文件。"],
    "Root URL": ["根 URL"],
    "The root web address which is used to communicate with the Sentry backend.": ["web的根地址用来欲Sentry后台通信。"],
    "Admin Email": ["管理员 Email 地址"],
    "The technical contact for this Sentry installation.": ["该Sentry服务中的技术支持"],
    "Support Email": ["支持邮箱"],
    "The support contact for this Sentry installation.": ["该Sentry服务中的支持联系人"],
    "Security Email": ["安全邮箱"],
    "The security contact for this Sentry installation.": ["该Sentry服务的安全联系人"],
    "Rate Limit": ["速率限制"],
    "The maximum number of events the system should accept per minute. A value of 0 will disable the default rate limit.": ["一分钟能接受的最多事件量。设置为0则禁用速率限制。"],
    "Allow Registration": ["允许注册"],
    "Allow anyone to create an account and access this Sentry installation.": ["允许任何人创建账户并访问该Sentry服务。"],
    "IP Rate Limit": ["IP 速率限制"],
    "The maximum number of times an authentication attempt may be made by a single IP address in a 60 second window.": ["单个IP在60秒内最多能尝试鉴权的次数。"],
    "User Rate Limit": ["用户速率限制"],
    "The maximum number of times an authentication attempt may be made against a single account in a 60 second window.": ["单个用户60秒内最多能尝试鉴权的次数。"],
    "The maximum number of organizations which may be created by a single account in a one hour window.": ["单个用户一个小时内最多能创建的组织数量。"],
    "Email From": ["电子邮件来自"],
    "Email address to be used in From for all outbound email.": ["邮件使用所有发信邮件的发件人的地址。"],
    "Outbound email": ["发出邮件"],
    "SMTP Host": ["SMTP 主机"],
    "SMTP Port": ["SMTP 端口"],
    ...
    "2 hours": ["2 小时"],
    "4 hours": ["4 小时"],
    "Looking for your slack channel (this can take a while)": ["正在查找您的Slack频道（这可能需要一段时间）"],
    "Threshold Type": ["阈值类型"],
    "Total Sessions": ["全部会话"],
    "Total Users": ["总用户"],
    "Slack": ["Slack"],
    ...
    "Deleting this issue is permanent. Are you sure you wish to continue?": ["问题将永久删除，您确定要继续吗？"],
    "Subscribe": ["订阅"],
    "Unsubscribe": ["取消订阅"],
    "Share": ["分享"],
    ...
    "These events will be unmerged and grouped into a new issue. Are you sure you want to unmerge these events?": ["这些事件将被分离并合并放入一个新的问题。您确定要分离这些事件？"],
    "Unmerge": ["分离"],
    "Compare": ["对比"],
    "Expand All": ["展开全部"],
    "Collapse All": ["折叠全部"],
    ...
    "This is an experimental feature. Data may not be immediately available while we process merges.": ["这是一个实验功能。当我们处理合并时数据不会立刻生效。"],
    "Are you sure you want to merge these issues?": ["你确定要合并这些问题？"],
    "Tags are automatically indexed for searching and breakdown charts. Learn how to [link: add custom tags to issues]": ["为了搜索和做分解图，标签自动会被索引。了解更多[link: 向问题中添加自定义标签]"],
    "More": ["更多"],
    ...
    "Hide Details": ["隐藏详情"],
    "Show Details": ["显示详情"],
    "This identifier is unique across your organization, and can be used to reference an issue in various places, like commit messages.": ["该标识符在您的组织中是唯一的，可以用于在各种地方引用问题，例如提交消息。"],
    ...
    "This will apply to the first [bulkNumber] issues matched in this project!": ["这将应用于该项目前[bulkNumber]个匹配的问题！"],
    "This will apply to all [bulkNumber] issues matched in this project!": ["这将应用于该项目全部[bulkNumber]个匹配的问题！"],
    "Graph:": ["图形："],
    "24h": ["24 小时"],
    "Selected up to the first [count] issues that match this search query.": ["最多选择与该搜索查询匹配的前[count]个问题。"],
    "Selected all [count] issues that match this search query.": ["选择与该搜索查询匹配的全部[count]个问题。"],
    "Select the first [count] issues that match this search query.": ["选择与该搜索查询匹配的前[count]个问题。"],
    "Select all [count] issues that match this search query.": ["选择与该搜索查询匹配的全部[count]个问题。"],
    "Are you sure you want to [action] the first [bulkNumber] issues that match the search?": ["您确定要[action]该项目前[bulkNumber]个匹配搜索的问题？"],
    "Are you sure you want to [action] all [bulkNumber] issues that match the search?": ["您确定要[action]该项目全部[bulkNumber]个匹配搜索的问题？"],
    "Bulk deletion is only recommended for junk data. To clear your stream, consider resolving or ignoring. [link:When should I delete events?]": ["批量删除仅建议用于垃圾数据。考虑使用解决和忽略来清理你的流。[link:我什么时候应该删除事件？]"],
    "Custom Search": ["自定义搜索"],
    ...
    "The organization you were looking for was not found.": ["您要查找的组织未找到。"],
    "Create Organization": ["创建组织"],
    "Create a New Organization": ["创建新组织"],
    "Organizations represent the top level in your hierarchy. You'll be able to bundle a collection of teams within an organization as well as give organization-wide permissions to users.": ["在你的分级系统中，组织为最高级。你可在组织下绑定多组团队，并向用户发放组织内许可。"],
    "Organization Name": ["组织名称"],
    "e.g. My Company": ["例如：我的公司"],
    "I agree to the [termsLink:Terms of Service] and the [privacyLink:Privacy Policy]": ["我同意[termsLink:服务条款]和[privacyLink:隐私政策]"],
    "Request to join failed": ["申请加入失败"],
    "Request Sent": ["请求已发送"],
    "Email Address": ["Email 地址"],
    "The [organization] organization is currently in the process of being deleted from Sentry.": ["[organization]组织正在被从Sentry中删除。"],
    "Deletion Scheduled": ["计划删除"],
    "The [organization] organization is currently scheduled for deletion.": ["[organization]组织正在被排期删除。"],
    ...
    "If this is a mistake, contact an organization owner and ask them to restore this organization.": ["如果发生了错误，请联系组织的所有者并让他们恢复该组织。"],
    "Note: Restoration is available until the process begins. Once it does, there's no recovering the data that has been removed.": ["注意：在该过程开始之前重置都是可用的。一旦开始，被删除的数据都不能恢复。"],
    "Sorry, but this organization is currently in progress of being deleted. No turning back.": ["抱歉，该组织正在删除，无法撤消。"],
    "Team: ": ["团队:"],
    "Environment:": ["环境变量:"],
    ...
    "You can manage your own applications via the [link:API dashboard].": ["您可以通过[link:API仪表盘]管理您的应用。"],
    "You haven't approved any third party applications.": ["您没有批准任何第三放应用。"],
    "Closing your account is permanent and cannot be undone": ["关闭您的账户是永久且不可撤销的"],
    "Closing Account": ["正在关闭账户"],
    "Your account has been deactivated and scheduled for removal.": ["您的账户无效，而且已列入移除计划。"],
    "Thanks for using Sentry! We hope to see you again soon!": ["感谢使用 Sentry！希望很快能再次见到您！"],
    "Goodbye": ["再见"],
    ...
    "Want to change how many emails you get? Use the notifications panel.": ["希望改变收到的邮件数目？请使用通知面板。"],
    "Integration": ["集成"],
    "Disconnect": ["断开"],
    "This action can't be undone.": ["这个操作不可撤销。"],
    "Search Projects": ["搜索项目"],
    "Rotate Secret Key": ["轮替密钥"],
    "Two-factor authentication is required for at least one organization you're a member of.": ["双重认证要求你至少是一个组织的成员。"],
    "Created at": ["创建于"],
    "Last used": ["最近使用"],
    "Confirmation codes are sent to the following phone number": ["验证码被发送到下列手机号"],
    ...
    "Make sure to save a copy of your recovery codes and store them in a safe place.": ["请确保保存一份你回复码的备份并将其保存在一个安全的地方。"],
    "You have no more recovery codes to use": ["你没有更多恢复码可用"],
    "Removing the last authentication method will disable two-factor authentication completely.": ["移除最后一个认证方法将完全禁用双重认证。"],
    ...
    "Signing out of all devices will sign you out of this device as well.": ["登出全部设备也将会使你登出这个设备。"],
    ...
    "Authentication tokens allow you to perform actions against the Sentry API on behalf of your account. They're the easiest way to get started using the API.": ["授权令牌允许基于你的账户使用 Sentry API。这是简单的使用 API 的开始方式。"],
    "For more information on how to use the web API, see our [link:documentation].": ["更多关于如果使用我们的web API， 请参照 [link:文档]。"],
    "Create Token": ["创建令牌"],
    "Token": ["令牌"],
    "Scopes": ["作用域"],
    "Removed token": ["移除令牌"],
    ...
    "Auth Token": ["认证令牌"],
    "You haven't created any authentication tokens yet.": ["你还没创建任何授权令牌。"],
    "Change your account details and preferences (e.g. timezone/clock, avatar, language)": ["改变你的账户详情和偏好（例如：时区/时钟，头像，语言）。"],
    "Change your account password and/or two factor authentication": ["改变你的账户密码和/或双重认证"],
    "Configure what email notifications to receive": ["配置接受什么邮件通知"],
    "Add or remove secondary emails, change your primary email, verify your emails": ["添加或移除次要邮箱，改变主要邮箱，验证邮箱"],
    "Authorized Applications": ["认证过的应用"],
    "Manage third-party applications that have access to your Sentry account": ["管理关联在Sentry账户上的第三方应用"],
    "Manage your third-party identities that are associated to Sentry": ["管理关联在Sentry上的第三方身份"],
    "Permanently close your Sentry account": ["永久关闭你的Sentry账户"],
    "Add and configure OAuth2 applications": ["添加并配置OAuth2应用"],
    "On": ["开"],
    "Off": ["关"],
    "Email Routing": ["邮件路由"],
    "On a per project basis, route emails to an alternative email address.": ["在每个项目的基础上，将电子邮件路由到另一个电子邮件地址。"],
    "Workflow Notifications": ["工作流程通知"],
    "Control workflow notifications, e.g. changes in issue assignment, resolution status, and comments.": ["设置工作流程通知，例如 issue 指派变更、解决状态变更以及有新评论时。"],
    "Always": ["总是"],
    "Only on issues I subscribe to": ["仅在我关联的问题上"],
    "Never": ["永不"],
    "Deploy Notifications": ["部署通知"],
    "Control deploy notifications that include release, environment, and commit overviews.": ["控制包括版本、环境和提交总览的部署通知。"],
    "Only on deploys with my commits": ["仅在有我的提交的部署上。"],
    "Weekly Reports": ["周报告"],
    "Reports contain a summary of what's happened within the organization.": ["报告包括一份组织里发生事情的总结。"],
    "Notification Settings": ["通知设置"],
    "Looking to add or remove an email address? Use the emails panel.": ["使用邮件面板来添加或删除电子邮件地址"],
    "Change password": ["修改密码"],
    "Changing your password will invalidate all logged in sessions.": ["修改密码将会作废全部已经登录的会话。"],
    "Add Rule": ["添加规则"],
    "Select type": ["选择类型"],
    ...
    "Get started with Single Sign-on for your organization by selecting a provider. Read more in our [link:SSO documentation].": ["从选择一个身份认证提供程序开启单点登录。更多关于SSO的信息请查阅[link:OSS文档]。"],
    ...
    "Removing the organization, [name] is permanent and cannot be undone! Are you sure you want to continue?": ["删除组织，[name]是永久且不可撤销的！您确定要继续吗？"],
    "This will also remove the following associated projects:": ["下列关联项目也将一同被删除："],
    "Remove Organization": ["移除组织"],
    "Removing this organization will delete all data including projects and their associated events.": ["删除组织将会删除包括项目和与之关联的事件在内的全部数据。"],
    "Created By": ["创建于"],
    "%s added": ["%s 已添加"],
    "Add Services": ["添加服务"],
    ...
    "Type root path of your source code, e.g. `src/`.": ["输入你的源码根目录，例如: `src/`。"],
    "Repo": ["仓库"],
    "Choose repo": ["选择仓库"],
    "Branch": ["分支"],
    ...
    "Join request": ["加入请求"],
    "Are you sure you want to invite [email] to your organization?": ["你确定要邀请[email]到你的组织吗？"],
    "Saved": ["已保存"],
    "Could not save...": ["无法保存..."],
    ...
    "Are you sure you want to disable all two-factor authentication methods for [name]?": ["您确定要禁用 [name] 的所有双因子认证方法吗？"],
    "Save Member": ["保存成员"],
    "Resend invite": ["重新发送邀请"],
    "Resend SSO link": ["重新发送 SSO 链接"],
    "2FA Enabled": ["双因子验证已启用"],
    "2FA Not Enabled": ["双因子验证未启用"],
    "Are you sure you want to remove [name] from [orgName]?": ["您确定要将[name]从[orgName]中移除吗？"],
    ...
    "No projects found.": ["没有找到任何项目。"],
    "Adjust Limits": ["调整限额"],
    "Rate limits allow you to control how much data is stored for this\n                organization. When a rate is exceeded the system will begin discarding\n                data until the next interval.": ["速度限制可以控制为这个组织存储多少数据。当速度超过时系统开始丢弃数据直到下个时间周期。"],
    "No Limit": ["无限制"],
    "[number] per hour": ["[number]每小时"],
    "The maximum number of events to accept across this entire organization.": ["在整个组织中可以接受的最大事件数。"],
    "Account Limit": ["账户限额"],
    "Your account is limited to a maximum of [maxRate] events per [maxRateInterval] seconds.": ["您的账户被限制每[maxRateInterval]秒最多[maxRate]事件。"],
    ...
    "[requesterName] requests to add [name] to the [team] team.": ["[requesterName]请求将[name]加入[team]团队。"],
    ...
    "There was an error while trying to remove a member from the team.": ["从团队中移除成员时出错。"],
    "No members": ["没有成员"],
    "Add Member": ["添加成员"],
    "Unable to load organization members.": ["加载组织成员失败。"],
    "You do not have enough permission to associate a project.": ["您没有足够的权限关联项目。"],
    "You do not have enough permission to change project association.": ["您没有足够的权限变更项目关联。"],
    ...
    "Team Settings": ["团队设置"],
    "This may affect team members' access to projects and associated alert delivery.": ["这个将会影响团队成员对项目的访问及告警发送。"],
    "Are you sure you want to remove the team [team]?": ["您确定您要移除团队[team]？"],
    "There are no keys active for this project.": ["这个项目没有活动的秘钥。"],
    "View, enable, and disable all integrations for a project": ["查看，开启和禁用项目的全部集成"],
    ...
    "Configure a project's inbound filters (e.g. browsers, messages)": ["创建项目进入过滤器（例如：浏览器、消息）"],
    "Debug Files": ["调试文件"],
    "ProGuard": ["ProGuard"],
    "SDK Setup": ["SDK安装"],
    "Client Keys (DSN)": ["客户端密钥 (DSN)"],
    "View and manage the project's client keys (DSN)": ["查看和管理项目客户端秘钥（DSN）"],
    ...
    "Undiscarding this issue means that incoming events that match this will no longer be discarded. New incoming events will count toward your event quota and will display on your issues dashboard. Are you sure you wish to continue?": ["不丢弃这个问题意味着匹配这个的到来的事件不再被丢弃。新到来的事件将会计算进你的事件配额并展示在问题仪表盘上。您确定要继续吗？"],
    "Undiscard": ["不丢弃"],
    "Events similar to these will no longer be filtered": ["与这些类似的事件将不会再被过滤出来"],
    ...
    "Release and Error Message filtering are not enabled on your Sentry installation": ["你的Sentry安装实例未允许根据发布版本和错误消息过滤"],
    "Filters": ["过滤器"],
    "Key Details": ["秘钥详情"],
    ...
    "Revoke Key": ["移除秘钥"],
    "Revoking this key will immediately remove and suspend the credentials. This action is irreversible.": ["吊销秘钥将立刻移除并暂停证书。这个动作不可逆转。"],
    "Are you sure you want to revoke this key? This will immediately remove and suspend the credentials.": ["您确定要吊销这个秘钥吗？将会立刻移除和暂停证书。"],
    "Revoking key…": ["撤销秘钥中..."],
    "Revoked key": ["撤销秘钥"],
    "Unable to revoke key": ["撤销秘钥失败"],
    "Your credentials are coupled to a public and secret key. Different clients will require different credentials, so make sure you check the documentation before plugging things in.": ["您的证书包含公钥和私钥。不同的客户端请求不同的证书，因此在插入前一定要检查文档。"],
    "Key usage in the last 30 days (by day)": ["近30天密钥的使用状况（按天）"],
    "Total events captured using these credentials.": ["使用这些凭据捕获的全部事件。"],
    ...
    "Remove Key": ["移除秘钥"],
    "Are you sure you want to remove this key? This action is irreversible.": ["您确定您要移除这个秘钥吗？这个动作时不可逆转的。"],
    "Secret Key": ["私钥"],
    "Project ID": ["项目ID"],
    "DSN": ["DSN"],
    "Hide deprecated DSN": ["隐藏废弃的 DSN"],
    "Show deprecated DSN": ["显示废弃的 DSN"],
    "Deprecated DSN includes a secret which is no longer required by newer SDK versions. If you are unsure which to use, follow installation instructions for your language.": ["废弃DSN包含新版本的SDK不再需要的秘钥。如果您不确定使用哪一个，参考您语言的安装说明。"],
    "DSN (Deprecated)": ["DSN（废弃）"],
    "Security Header Endpoint": ["安全头部端点"],
    "Minidump Endpoint": ["转储端点"],
    "Use this endpoint to upload [link], for example with Electron, Crashpad or Breakpad.": ["使用这个端点来上传[link]，例如：Electron、Crashpad和Breakpad。"],
    "Add File": ["添加文件"],
    "File": ["文件"],
    "Issue Owners": ["问题所有者"],
    "Configure release tracking for this project to automatically record new releases of your application.": ["为这个项目配置版本追踪可以自动记录您应用的新版本。"],
    "Client Configuration": ["客户端配置"],
    "This will annotate each event with the version of your application, as well as automatically create a release entity in the system the first time it's seen.": ["这将会为每个事件注释您应用的版本，同时当第一次出现时自动在系统中创建版本实体"],
    "In addition you may configure a release hook (or use our API) to push a release and include additional metadata with it.": ["此外您可以配置一个版本钩子（或使用API）来推送版本和额外的元信息。"],
    "Deploy Token": ["部署令牌"],
    "A unique secret which is used to generate deploy hook URLs": ["一个用于生成部署钩子URL的独立秘钥"],
    "If a service becomes compromised, you should regenerate the token and re-configure any deploy hooks with the newly generated URL.": ["如果服务被破坏，您需要重新生成令牌并使用新生成的URL重新配置部署钩子。"],
    "Regenerate Token": ["重新生成令牌"],
    "Are you sure you want to regenerate your token? Your current token will no longer be usable.": ["您确定要重新生成您的令牌？您当前的令牌将不能再使用。"],
    "Webhook": ["Webhook"],
    "If you simply want to integrate with an existing system, sometimes its easiest just to use a webhook.": ["如果您希望整合一个现有系统，有时最简单的方式是使用webhook。"],
    "The release webhook accepts the same parameters as the \"Create a new Release\" API endpoint.": ["版本webhook接受与\"创建新版本\"API相同的参数。"],
    "You can notify Sentry when you release new versions of your application via our HTTP API.": ["您的应用发布新版本的时候，您可以通过HTTP API通知Sentry。"],
    "See the [link:releases documentation] for more information.": ["更多信息请查看[link:版本API文档]。"],
    "Your deploy token has been regenerated. You will need to update any existing deploy hooks.": ["您的部署令牌已经重新生成。您需要更新全部已经存在的部署钩子。"],
    "Unable to regenerate deploy token, please try again": ["重新生成部署令牌失败，请重试"],
    "Event Validation": ["事件验证"],
    "Secret": ["秘钥"],
    "The shared secret used for generating event HMAC signatures.": ["分享秘钥用于生成事件的HMAC签名。"],
    "Delete Hook": ["删除钩子"],
    ...
    "There are no service hooks associated with this project.": ["这个项目没有关联的服务钩子。"],
    "Create Hook": ["创建钩子"],
    ...
    "Looking to fine-tune your personal notification preferences? Visit your Account Settings": ["想要更好的调整您的个人通知喜好?请查看您的账户设置"],
    "There are no integrations available for data forwarding": ["没有供数据转发的有效的集成。"],
    "Data Forwarding allows processed events to be sent to your\n                favorite business intelligence tools. The exact payload and\n                types of data depend on the integration you're using. Learn\n                more about this functionality in our [link:documentation].": ["数据转发将处理过的事件发送到你喜欢的业务集成工具。确切的数据载荷和类型取决于你使用的集成。在[link:文档]了解更多关于这个功能。"],
    "Forwarded": ["转发"],
    "Forwarded events in the last 30 days (by day)": ["近30天内转发的事件（按天）"],
    "Nothing forwarded in the last 30 days.": ["近30天内没有转发过数据。"],
    "Total events forwarded to third party integrations.": ["全部转发到第三方集成的事件。"],
    "You do not have permission to delete debug files.": ["您没有删除调试文件的权限。"],
    ...
    "Remove the [project] project and all related data. [linebreak] Careful, this action cannot be undone.": ["删除 [project] 项目并清除所有相关数据。 [linebreak] 注意此操作无法撤销！"],
    "Remove Project": ["删除项目"],
    "You do not have the required permission to remove this project.": ["你没有删除该项目的权限。"],
    "This project cannot be removed. It is used internally by the Sentry server.": ["该项目由 Sentry 服务器内部使用，无法删除。"],
    "Removing this project is permanent and cannot be undone!": ["移除该项目的操作是永久的，无法撤销！"],
    "This will also remove all associated event data.": ["该操作也将移除所有相关事件数据。"],
    "Transfer the [project] project and all related data. [linebreak] Careful, this action cannot be undone.": ["转让 [project] 项目以及所有相关数据。[linebreak] 注意此操作无法撤销！"],
    "You do not have the required permission to transfer this project.": ["您没有权限转让本项目。"],
    "This project cannot be transferred. It is used internally by the Sentry server.": ["这个项目不能被转移。它被Sentry服务内部使用。"],
    "Transfer project": ["转让项目"],
    "Transferring this project is permanent and cannot be undone!": ["转让此项目是永久性的，无法撤消！"],
    "Organization Owner": ["组织所有者"],
    "Event Settings": ["事件设置"],
    "Client Security": ["客户端安全性"],
    "Configure origin URLs which Sentry should accept events from. This is used for communication with clients like [link].": ["配置Sentry接受事件的原始URL。这用于与类似[link]的客户端通信。"],
    "This will restrict requests based on the [Origin] and [Referer] headers.": ["这将限制基于[Origin]和[Referer]头的请求。"],
    ...
    "To configure [csp:CSP] reports\n              in Sentry, you'll need to send a header from your server describing your\n              policy, as well specifying the authenticated Sentry endpoint.": ["在Sentry中配置[csp:CSP]报告，您需要从您的服务器发送一个头部来描述您的策略，同时指定认证过的Sentry端。"],
    "For example, in Python you might achieve this via a simple web middleware": ["例如，在Python中您可以通过一个web中间件来实现"],
    ...
    "To configure reports in Sentry, you'll need to configure the [header] a header from your server:": ["在Sentry中配置报告，您需要配置服务器[header]头部："],
    "For more information, see [link:the article on MDN].": ["更多信息，请查看[link:MDN上的文章]。"],
    ...
    "To configure HPKP reports\n              in Sentry, you'll need to send a header from your server describing your\n              policy, as well specifying the authenticated Sentry endpoint.": ["为了在Sentry中配置HPKP报告，您需要从服务器发送一个头部来表述你的策略，同时指定认证过的Sentry端。"],
    "Alternatively you can setup HPKP reports to simply send reports rather than\n              actually enforcing the policy": ["或者，您可以将HPKP报告设置为仅发送报告，而不实际执行策略"],
    "We recommend setting this up to only run on a percentage of requests, as\n              otherwise you may find that you've quickly exhausted your quota. For more\n              information, take a look at [link:the documentation on MDN].": ["我们建议将其设置为仅在一定比例的请求上运行，否则您将会发现您的配额被快速耗尽。更多信息请查看[link:MDN上的文档]。"],
    "Security Header Reports": ["安全头部报告"],
    "Additional Configuration": ["附加配置"],
    "In addition to the [key_param] parameter, you may also pass the following within the querystring for the report URI:": ["除了[key_param]参数，您还可以在报告URI的查询字符串中传递以下内容："],
    "The version of the application.": ["应用的版本"],
    "Supported Formats": ["支持的格式"],
    "Instructions": ["说明"],
    "Report URI": ["报告 URI"],
    "We've automatically pulled these credentials from your available [link:Client Keys]": ["我们已经自动从你可用的[link:客户端秘钥]拉取了这些证书。"],
    "Artifacts": ["工件"],
    "Artifact": ["工件"],
    ...
    "Each event in Sentry may be annotated with various tags (key and value pairs).\n                 Learn how to [link:add custom tags].": ["在Sentry中每个事件都被各个标签（键值对）注释着。了解如何[link:添加自定义标签]。"],
    "There are no tags, [link:learn how to add tags]": ["没有标签。[link:了解添加标签]"],
    ...
    "You are about to unsubscribe from project notifications for the following project:": ["你将取消订阅下列项目的通知："],
    "You can subscribe to it again by going to your account settings.": ["在你的账户设置中，你可以重新订阅。"],
    "See an example": ["例如"],
    "Sorry, no user reports match your filters.": ["抱歉，没有满足您过滤条件的事件。"]
  },
  "untranslated": {
    "Recommended": [""],
    "Latest": [""],
